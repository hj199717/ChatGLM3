...
# def upload_file(file):#接受一个文件作为参数 file，然后将其打印输出并返回该文件。
#     print(file)
#     return file

def add_text(history, text,change):
    ...
    if len(change) ==1:
        ...
        change = ",".join(change)
    history = history + [( text + "并结合"+change +"进行倾向预测。", None)]
    return history, gr.update(value="", interactive=False)

def bot(history,model_names,bar):
    """
    聊天调用的函数
    :param history:
    :return:
    ...
    if isinstance(message, tuple):
        ...
        kg_one,kg_attr = extract_user_mysql(user)
        ...
        #
        text_attr="""请根据以下知识完成回答，回答内容必须严格遵守知识约束:{}，回答问题：请输出文本{}中的对应字段，作为用户的基本属性信息""".format(kg_attr,kg_attr)
        text_id="""请根据以下知识完成回答，回答内容必须严格遵守知识约束:{}，回答问题：请计算文本{}中，'用户id'出现的次数，只需要回答一个数值大小即可，不需要其他内容""".format(kg_one,kg_one)
        text_location="""请根据以下知识完成回答，回答内容必须严格遵守知识约束:{}，回答问题：请计算文本{}中，出现的地理位置，只需要返回出现的地理位置即可，不需要其他内容""".format(kg_location,kg_location)
        text_content="""请根据以下知识完成回答，回答内容必须严格遵守知识约束:{},回答问题：请概括一下{}的关键词，输出最具代表性的3个关键词""".format(kg_content,kg_content)
        text_emotion="""请根据以下知识完成回答，回答内容必须严格遵守知识约束:{}，回答问题：请问列表{}中的内容是什么？，只需输出列表中的字符，不需要其他内容""".format(kg_emotion,kg_emotion)
        text_aite="""请根据以下知识完成回答，回答内容必须严格遵守知识约束:{}，回答问题：请问{}中的内容是什么？""".format(kg_aite,kg_aite)
        ...
        # response_id = detailed_report_user(model_names,bar,text_id)
        # print("response_id_test++++++++++++++++++++++",list(response_id)[-1])
        # response_location = list(detailed_report_user(model_names,bar,text_location))[-1]
        # response_content = list(detailed_report_user(model_names,bar,text_content))[-1]
        # response_emotion = list(detailed_report_user(model_names,bar,text_emotion))[-1]
        # response_aite = list(detailed_report_user(model_names,bar,text_aite))[-1]


        texts="""请主要参考提供以下知识完成回答，回答内容必须严格遵守知识约束：以下是用户"{}"的属性信息{}，近期的微博行为数据:"用户地理位置：{}，用户发文次数：{},发文关键词：{},发文倾向性为：{}，提及好友为：{}"，请根据上述知识，回答问题：{}，
        只需要输出用户性别，用户等级，粉丝数，关注数，互关数，创建时间，是否认证，认证类型，用户地理位置，用户发文次数，发文关键词，发文倾向，提及好友是谁即可，如果没有，直接回答无即可。并基于上述信息，结合政治方面，对用户进行深层次分析""".format(user,text_attr,response_location,response_id,response_content,response_emotion,response_aite,text)
        
        history[-1][1] = model.chat(tokenizer, texts, history=[])[0]
        # history[-1][1] = list(detailed_report_user(model_names,bar,texts))[-1]
    ...
    with gr.TabItem("用户画像系统"):
        with gr.Row():
            with gr.Column():
                ...
                bar = gr.Slider(label='大模型温度', value=0.1, minimum=0, maximum=1, step=0.05, container=True,interactive=True)
    ...
    with gr.TabItem("用户匹配系统"):
        ...
        with gr.Row():
            with gr.Column(scale=1, min_width=200):
                ...
                bar = gr.Slider(label='大模型温度', value=0, minimum=0, maximum=1, step=0.05, container=True)
                model2 = gr.Dropdown(label='大模型选择', value='gemma:7b-instruct-fp16', allow_custom_value=True,
                                    choices=['gemma:7b-instruct-fp16',
                                             # 'gemma:7b-text-fp16',
                                             'qwen:72b',
                                             'ChatGLM3-6B',
            ...
            with gr.Column(scale=8):
                ...
                llm_button.click(detailed_report, [model2, bar, user_name, match_name,plat1,plat2], [llm_res])
