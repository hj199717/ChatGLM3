...
def plat_change(plat):
    user_name = gr.Dropdown(label=f"输入查询用户名（{plat}）平台",)
#
...
with gr.Blocks() as demo:
    with gr.Row():
        with gr.Column(scale=2, min_width=200):
            ...
            plat = gr.Dropdown(label='平台', value='微博', allow_custom_value=True,interactive=True,
            ...
            model = gr.Dropdown(label='大模型选择', value='gemma:7b-instruct-fp16', allow_custom_value=True,interactive=True,
        ...
        with gr.Column(scale=9):
            with gr.Group():
                with gr.Row():
                    ...
                    user_name = gr.Dropdown(label="输入查询用户名",
                                            value='法外微型博士',
                                            allow_custom_value=True,
                                            choices=['星月打捞客', 'znl李先生', '巧克力蛋糕和你', '一大只爪Zhua_',
                                                     '咩咩ttwtbm', '太阳与七角', '青椒猫丝可不能吃',
                                                     '__美丽心情19105', '今天喝白开水了嘛', '圣墓山神棍',
                                                     '小林画橙子', '新鬼武者_9808',
                                                     '风轻云淡特好看123', '黄埔检察', '跟他妈做梦似的',
                                                     '划闻轶事', '坟场碎尸少女'])
            ...
            llm_res = gr.Chatbot(label="用户画像分析报告", height=650)
            ...
            plat.change(plat_change, plat, user_name)
