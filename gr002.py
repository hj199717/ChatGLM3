...
def detailed_report(model_selection,temperature,user1, user2):
    ...
    partial_message = ""
    for response in client.generate(model=model_selection, prompt=myprompt, options={'temperature': temperature, }, stream=True, ):
        ...
        with gr.Column(scale=9):
            with gr.Group():
                with gr.Row():
                    ...
                    user_name = gr.Dropdown(label="待匹配用户名",
                                            # value='老杨ysh_73',
                                            allow_custom_value=True,
                        choices=['老杨ysh_73','znl李先生','SOD_大宝','冷温柔Triste998','禅是禅非','Bigg宇宙','陈曦guitar',
                        '马丽lili','天翼帐号11018428363','G友116987631','炫铃用户2911236035','多米音乐_8196567',
                        '鹤壁vivoXpiay','贝壳手机用户2925697','傲雪寒梅1213','王泽Happy','夕阳beautiful'])

                    match_button = gr.Button("进行匹配")
                    #match_name = gr.Textbox(label="匹配成功的用户名")
                    match_name = gr.Dropdown(label="匹配成功的用户名",interactive=True,allow_custom_value=True)
