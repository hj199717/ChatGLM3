...
def bot(history):
    ...
    if isinstance(message, tuple):
        ...
        text_location="""请根据以下知识完成回答，回答内容必须严格遵守知识约束:{}，回答问题：请计算文本{}中，'用户id'出现的次数，只需要回答一个数值大小即可，不需要其他内容""".format(kg_location,kg_location)
        text_content="""请根据以下知识完成回答，回答内容必须严格遵守知识约束:{},回答问题：请概括一下{}的关键词，输出最具代表性的3个关键词""".format(kg_content,kg_content)
        text_emotion="""请根据以下知识完成回答，回答内容必须严格遵守知识约束:{}，回答问题：请问{}中的内容是什么？，不需要其他内容""".format(kg_emotion,kg_emotion)
